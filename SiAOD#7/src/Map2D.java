public class Map2D {
        ...
        public Map2D(int width, int height)
        {
            if (width <= 0 || height <= 0)
            {
                throw new IllegalArgumentException(
                        "длина и высота должны быть положительны; имеем " + width +
                                "x" + height);
            }
            this.width = width;
            this.height = height;
        ...
        private void checkCoords(int x, int y)
        {
            ...
            if (y < 0 || y > height)
            {
                throw new IllegalArgumentException("y должна быть в диапазоне [0, " +
                        height + "), имеем " + y);
            }
        }
        ...
        {
            if (loc == null)
                throw new NullPointerException("локацияя не может быть нулевой");
